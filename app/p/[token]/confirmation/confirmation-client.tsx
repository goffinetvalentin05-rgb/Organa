...
export function PublicPlanningConfirmationClient() {
    ...
    const load = async () => {
      ...
      if (assignmentIdFromUrl && /^[0-9a-f-]{36}$/i.test(assignmentIdFromUrl)) {
        try {
          ...
          if (!res.ok) {
            if (!cancelled) {
              setLoadError(data?.error || "Impossible de charger la confirmation");
            }
            return;
          }
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed?.assignmentId && parsed?.slotDate && !cancelled) {
            setPayload(parsed);
            return;
          }
        }
  ...
  if (loadError) {
    return (
      <div className="min-h-screen bg-slate-50 py-8 px-4">
        <div className="max-w-lg mx-auto rounded-xl border border-rose-200 bg-rose-50 px-6 py-4 text-rose-800 text-sm">
          {loadError}
        </div>

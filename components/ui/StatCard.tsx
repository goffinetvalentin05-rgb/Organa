...
export default function StatCard({ label, value, icon: Icon, footer, href, className }: StatCardProps) {
  const inner = (isInteractive: boolean) => (
    <>
      ...
      {footer ? (
        <div className="mt-3 text-sm text-slate-600 [&_.font-medium]:text-slate-800 [&_span]:text-inherit">
          {footer}
        </div>
      ) : null}
    </>
  ...
  if (href) {
    return (
      <Link
        href={href}
        className={cn(glassCardClass, "group block p-6 shadow-xl shadow-blue-950/10", interactive, className)}
      >
        {inner(true)}
      </Link>
    );
  ...
  return <div className={cn(glassCardClass, "p-6 shadow-xl shadow-blue-950/10", className)}>{inner(false)}</div>;
}

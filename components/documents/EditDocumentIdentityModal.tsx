...
export default function EditDocumentIdentityModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isQuote) {
      if (!ti) {
        setLocalError(t("dashboard.documents.identityEdit.errorEmptyTitle"));
        return;
      }
      if (ti.length > DOCUMENT_TITLE_MAX_LENGTH) {
    ...
    try {
      const res = await fetch("/api/documents", {
        ...
        body: JSON.stringify(
          isQuote
            ? { id: documentId, type, title: ti }
            : { id: documentId, type, numero: n, title: ti }
        ),
      });
      ...
      onSaved({
        numero: data.numero ?? (isQuote ? initialNumero : n),
        title: data.title ?? ti,
      });
    } catch (err: unknown) {
      const message =
          ...
          <div>
            ...
            {isQuote ? (
              <div
                ...
                className="input-obillz w-full cursor-default font-medium opacity-90"
                aria-readonly
              >
                {numero}
              </div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              {isQuote
                ? t("dashboard.documents.identityEdit.referenceAutoGenerated")
